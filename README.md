...
| GET    | [api/items](server/routes/items.ts#L9)                               | query: — body: —                    | `Item[]` (`id`, `institution_name`, `created_at` only; no Plaid access token) |
| GET    | [api/accounts](server/routes/accounts.ts#L7)                           | query: — body: —                    | authenticated user `Account[]` with `institution_name` for react-admin lists |
| GET    | [api/:itemId/accounts](server/routes/accounts.ts#L19)                  | query: — body: —                    | `Account[]`                                                    |
| POST   | [api/:itemId/accounts/refresh](server/routes/accounts.ts#L24)          | query: — body: —                    | `{ success: true, item_id, updated_accounts }` (calls Plaid `accounts/get`; updates only `name`, `official_name`, `balances`; returns `409` if DB/Plaid account counts mismatch) |
