...
export function getSummary(txns: Txn[]) {
...
export type TxnVirtualizedTableProps = {
  transactions: Txn[];
  /** Total rows before filter (for “n of m shown”) */
  sourceTotal?: number;
  header?: ReactNode;
  noneMatchHint?: ReactNode;
  emptyMessage?: string;
  keyPrefix?: string;
  taggingMode?: boolean;
  nettingMode?: boolean;
...
export function buildDisplayRows(transactions: Txn[], nettingMode: boolean, keyPrefix: string): DisplayRow[] {
...
export default function TxnVirtualizedTable({
  transactions,
  sourceTotal,
  header,
  noneMatchHint,
  ...
  const total = sourceTotal ?? transactions.length;
  ...
  if (!transactions.length && total === 0) return <div className="muted">{emptyMessage}</div>;
  ...
  return (
    <>
      ...
      <div className="row-flex flex-wrap gap-4 mb-3 small">
        <span>Income: <strong>{fmtUsd(summary.income)}</strong></span>
        <span>Spending: <strong>{fmtUsd(summary.spending)}</strong></span>
        <span><strong>{summary.count.toLocaleString()}</strong> of {total.toLocaleString()} shown</span>
      </div>
