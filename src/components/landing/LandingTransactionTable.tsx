...
type ColumnId = "actions" | "date" | "tags" | "name" | "merchant" | "amount" | "account" | "detected" | "reset";
...
const DATA_COLUMNS: ColumnDef[] = [
  ...
  { id: "tags", label: "Tags", minWidth: 148 },
  { id: "account", label: "Account", minWidth: 150 },
  { id: "detected", label: "Detected", minWidth: 140 }
];
const DEFAULT_HIDDEN: ColumnId[] = ["merchant"];
const clampWidth = (w: number, min: number) => Math.max(min, Math.round(w));
const defaultPrefs = (cols: ColumnDef[]): ColumnPrefs => ({ order: cols.map((c) => c.id), widths: {}, hidden: [...DEFAULT_HIDDEN] });
...
const Row = memo(function Row({ t, tagMap, measureRef, dataIndex, columns }: RowProps) {
  return (
    <tr ref={measureRef} data-index={dataIndex}>
      {columns.map((col) => {
        const cellCls = col.fixed ? "txn-col-fixed" : undefined;
        if (col.id === "actions") return <td key={col.id} className={cellCls}>{getTxnIconUrl(t) && <img src={getTxnIconUrl(t)} alt="" style={{ width: 22, height: 22, borderRadius: 6 }} />}</td>;
        if (col.id === "reset") return <td key={col.id} className={cellCls} />;
        if (col.id === "date") return <td key={col.id} className="text-nowrap">{formatTxnDate(t)}</td>;
        if (col.id === "tags") return <td key={col.id}><Badges badges={tagBadges(t, tagMap)} /></td>;
        if (col.id === "name") return <td key={col.id}>{(t.original_description || "").trim() || t.name || ""}</td>;
        if (col.id === "merchant") return <td key={col.id}>{t.merchant_name || ""}</td>;
        if (col.id === "amount") return <td key={col.id} className={`text-end ${(t.amount ?? 0) < 0 ? "money-positive" : ""}`}>{formatTxnAmount(t)}</td>;
        if (col.id === "account") return <td key={col.id}>{accountDisplay(t.institution_name || "", t.account_name || t.account_official_name || "")}</td>;
        return <td key={col.id}>{formatTxnDetectedCategory(t.personal_finance_category)}</td>;
      })}
    </tr>
  );
});
...
export default function LandingTransactionTable({ transactions, emptyMessage = "No transactions", keyPrefix = "txn", tags = [] }: Props) {
  const scrollRef = useRef<HTMLDivElement>(null);
  const headerRefs = useRef<Partial<Record<ColumnId, HTMLTableCellElement | null>>>({});
  const tagMap = useMemo(() => new Map(tags.map((t) => [t.id, t])), [tags]);
  const displayRows = useMemo(() => transactions.map((t, i) => ({ t, id: t.transaction_id || `${keyPrefix}-${i}` })), [transactions, keyPrefix]);
  ...
  const [prefs, setPrefs] = useState<ColumnPrefs>(() => defaultPrefs(baseColumns));
  ...
  useEffect(() => {
    ...
    const widths = Object.fromEntries(orderedColumns.filter((c) => !c.fixed).map((c) => [c.id, clampWidth(headerRefs.current[c.id]?.offsetWidth ?? c.minWidth, c.minWidth)])) as Partial<Record<ColumnId, number>>;
    setPrefs((p) => ({ ...p, widths }));
  ...
  const resetColumns = () => setPrefs(defaultPrefs(baseColumns));
  ...
  if (!transactions.length) {
    return (
      <>
        <div className="txn-table-toolbar row-flex flex-wrap gap-4 mb-3 small">
          <span>Income: <strong>{fmt(0)}</strong></span>
          <span>Spending: <strong>{fmt(0)}</strong></span>
          <span><strong>0</strong> transactions</span>
        </div>
        <div className="table-wrap" style={{ maxHeight: "70vh", overflowY: "auto" }}>
          <div className="muted" style={{ padding: "var(--s4)" }}>{emptyMessage}</div>
        </div>
      </>
    );
  }
      ...
      <div className="table-wrap" ref={scrollRef} style={{ maxHeight: "70vh", overflowY: "auto" }}>
        <table className="table txn-table" style={{ tableLayout: Object.keys(prefs.widths).length ? "fixed" : "auto", width: Object.keys(prefs.widths).length ? orderedColumns.reduce((s, c) => s + (c.width ?? c.minWidth), 0) : "100%" }}>
          ...
          <tbody>
            ...
            {items.map((vi) => {
              const { t, id } = displayRows[vi.index];
              return (
                <Row
                  key={id}
                  t={t}
