...
export default function CheckboxFilter<T extends string>({ label, options, selected, onChange, showSelectAll = true, bare = false }: CheckboxFilterProps<T>) {
  ...
  return (
    <div>
      ...
      {bare ? inner : <div className="border rounded p-2" style={{ maxHeight: 150, overflowY: "auto" }}>{inner}</div>}
    </div>
  );
}
